...
// TODO O CÓDIGO AQUI
window.tomarMedicamento = function (pacientId, medicamentoNome, medicamentoHora) {
  const paciente = pacientes.find(p => p.id === pacientId);
...
document.getElementById('medicamento-form').addEventListener('submit', function (event) {
  ...
  paciente.adicionarMedicamentos(nome, total, dose, frequencia);
  savePaciente();
  renderTable();
});
